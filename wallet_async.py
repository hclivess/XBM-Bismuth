...
from Cryptodome.Hash import SHA, BLAKE2b
...
def fingerprint():
    ...
    for f in root.filename:
        with open(f, 'rb') as fp:
            data = BLAKE2b.new(data=fp.read()).hexdigest()
            d[os.path.split(f)[-1]] = data

    openfield.insert(INSERT, d)
...
def encrypt_fn(destroy_this):
    ...
    if password == password_conf:
        ...
        # ignore `Using variable 'public_key_readable' before assignment` exception
...
def send(amount_input, recipient_input, operation_input, openfield_input):
    ...
    if not address_validate(recipient_input):
        ...
        app_log.info("Data: {}".format(openfield_input))
        ...
        # ignore `verifier.verify is not callable` exception
        if verifier.verify(h, signature):
            ...
            while True:
                ...
                app_log.info("Client: {}".format(reply))
                if reply[-1] == "Success":
    ...
    def msg_received_get(addlist):
        ...
        for x in addlist:
            if x[11].startswith(("msg=", "bmsg=", "enc=msg=", "enc=bmsg=")) and x[3] == address:
                ...
                m = '{} From {} : {}\n'.format(
                        time.gmtime(Decimal(x[1])),
                        replace_regex(msg_address, "alias="),
                        msg_received_digest)
                msg_received.insert(INSERT,((time.strftime("%Y-%m-%d %H:%M:%S", m))))
...
def tokens():
    ...
    print('Tokens results:', tokens_results)
...
def refresh(address, s=None):
    ...
    try:
        statusget = status.get('statusget')
        ...
        stats_account = status.get('stats_account')
        ...
        app_log.info("Transaction address balance: {}".format(balance))

        block_get = status.get('block_get')
        bl_height = block_get[0]
        db_timestamp_last = block_get[1]
        hash_last = block_get[7]

        # check difficulty
        diff = status.get('diffget')
        # check difficulty
        ...
        mempool_total = status.get('mpget')
        # print(mempool_total)
        ...
        annverget = status.get('annverget')
...
def support_collection(sync_msg_var, version_var):
    ...
    # collection_box.insert(INSERT, "\nConnected to: {}".format(ip)) # removed for undefined variable `ip`
...
def themes(theme):
    ...
    if theme == "Barebone" or None:
        ...
        with PIL.Image.open("themes/{}.jpg".format(theme)) as main_bg:
            main_bg.resize((width_main, height_main), PIL.Image.ANTIALIAS)
